procedure irstack (filelist, fileout, flatout)

string 	filelist	{prompt="Input raw files"}
string	fileout		{prompt="Output file"}
string	flatout		{prompt="Output flat field"}
bool	darksub
...
begin
	...
	imdel("@stackr,@stacks")
	imcomb("@"//input, "stackflat", combine="median", reject="sigclip",scale="median")
	imstat("stackflat",fields="mode", > "stacktemp")
	files = "stacktemp"
	dummy = fscan(files,line)
	dummy = fscan(files,stmode)
	imarith("stackflat","/",stmode,"stackflat")	
	imarith("@"//input,"/","stackflat","@stackr")
	display("1r",1)
	...
	imdel(fileout)
	imcomb("@stacks",fileout,combine="average",reject="sigclip",scale="none",hthresh=INDEF)
	imdel(flatout)
	imcopy("stackflat",flatout)
	; del("stack*")
	display(fileout,1)
end
